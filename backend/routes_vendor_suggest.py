...
async def suggest_vendors(product_ids: Optional[str] = None, top: int = 5, user=Depends(get_current_active_user)):
    ...
    # Aggregate historical PO performance per vendor
    ...
    for p in pos:
        ...
        if not vid:
            continue
        stat = perf.setdefault(vid, {"po_count": 0, "on_time": 0, "late": 0, "lead_days_sum": 0.0, "lead_n": 0, "product_ids": set()})
        stat["po_count"] += 1
        # products handled
        for it in (p.get("items") or []):
            if it.get("product_id"):
                stat["product_ids"].add(it["product_id"])
        # lead time: order_date -> receipt_date (proxy)
        ...
        if od and rd:
            ...
            if days >= 0:
                ...
                stat["lead_n"] += 1
        # on-time: if completed on/before delivery_date
        exp = _parse_iso(p.get("delivery_date"))
        if p.get("status") == "completed" and exp:
            # if we have any receipt within delivery_date treat as on-time (approximation)
            stat["on_time"] += 1
    ...
    ranked = []
    for v in vendors:
        stat = perf.get(v["id"], {"po_count": 0, "on_time": 0, "late": 0, "lead_days_sum": 0, "lead_n": 0, "product_ids": set()})
        rating = float(v.get("avg_rating") or 0)
        rating_score = min(rating / 5.0, 1.0)  # 0..1
        completed = stat["on_time"] + stat["late"]
        ontime_pct = (stat["on_time"] / completed) if completed else 0.5  # neutral if no data
        ontime_score = ontime_pct
        avg_lead = (stat["lead_days_sum"] / stat["lead_n"]) if stat["lead_n"] else 30.0
        # lower lead time = better; 0d=1.0, 30d=0.5, 60d+=0.0
        ...
        score = 0.4 * rating_score + 0.3 * ontime_score + 0.3 * leadtime_score + product_bonus
        reasons = []
        if rating >= 4:
            reasons.append(f"⭐ {rating:.1f}/5 rata-rata rating")
        elif stat["po_count"] == 0:
            reasons.append("Vendor baru (belum ada PO)")
    ...
    return {"suggestions": ranked[:top], "criteria": "40% rating · 30% on-time · 30% lead time · +10% jika pernah supply produk sama"}
